...
> 문서 기반 기능 요구사항 대비 현재 프론트엔드 구현 현황 (2026-02-12 기준)
...
| 신청 상세 | `(dashboard)/applications/[id]/page.tsx` | ✅ 완료 | 타임라인 포함 |
| 신청 편집/재제출 | `(dashboard)/applications/[id]/edit/page.tsx` | ⚠️ 부분 | 피드백 상세 내용 미표시 |
| API Key 관리 | `(dashboard)/api-keys/page.tsx` | ✅ 완료 | 재발급/비활성화 |
| 사용자 프로필 | `(dashboard)/profile/page.tsx` | ❌ 미구현 | 헤더에서 링크 참조하지만 페이지 없음 |
| 사용자 설정 | `(dashboard)/settings/page.tsx` | ❌ 미구현 | 헤더에서 링크 참조하지만 페이지 없음 |
...
| 검토 이력 | `(dashboard)/reviews/history/page.tsx` | ❌ 미구현 | 사이드바에 메뉴 있지만 페이지 없음 |

### 관리자 (IT_ADMIN / SYSTEM_ADMIN)
| 페이지 | 경로 | 상태 | 비고 |
|--------|------|------|------|
| 관리자 대시보드 | `(dashboard)/dashboard/page.tsx` | ✅ 완료 | AdminDashboard 컴포넌트 |
| 라이센스 관리 | `(dashboard)/monitoring/licenses/page.tsx` | ✅ 완료 | 차트+테이블 |
...
| 알림 센터 페이지 | `(dashboard)/notifications/page.tsx` | ❌ 미구현 | 헤더 드롭다운만 존재 |
| 감사 로그 | `(dashboard)/admin/audit/page.tsx` | ❌ 미구현 | 페이지 없음 |
...
| 신청서 취소 | FR-043 | ❌ 미구현 | UI 및 Mock API 없음 |
| 신청서 복사(재신청) | FR-044 | ❌ 미구현 | UI 및 Mock API 없음 |
| 피드백 기반 수정/재제출 | FR-053 | ⚠️ 부분 | 재제출 가능하나 피드백 상세 내용 미표시 |
...
| API Key 복사 | FR-064 | ❌ 미구현 | 복사 버튼 없음 |
...
| 데이터 내보내기 (CSV/Excel/PDF) | FR-078,085 | ❌ 미구현 | 버튼만 존재, 핸들러 없음 |
| 통계 리포트 생성 | FR-089 | ❌ 미구현 | 별도 리포트 페이지 없음 |

### 알림 시스템
| 기능 | 요구사항 ID | 상태 | 비고 |
|------|------------|------|------|
| 시스템 내 알림 (인앱) | FR-094 | ⚠️ 부분 | 헤더 드롭다운만 존재, 전체 알림 페이지 없음 |
| 알림 읽음/안읽음 관리 | FR-094 | ✅ 완료 | 헤더에서 가능 |
| 알림 설정 (채널별) | FR-095 | ❌ 미구현 | 사용자별 알림 설정 UI 없음 |
| 알림 센터 (전체 목록) | FR-094 | ❌ 미구현 | 전체 알림 이력 페이지 없음 |
...
| 프로필 조회 | FR-010 | ❌ 미구현 | 프로필 페이지 없음 |
| 프로필 수정 | FR-011 | ❌ 미구현 | |
| 마이페이지 (활동 내역) | FR-013 | ❌ 미구현 | |
...
### Priority 1 — 미구현 페이지 (❌)
1. **프로필/마이페이지** — 사용자 프로필 조회/수정, 활동 내역, 알림 설정
2. **알림 센터 페이지** — 전체 알림 목록, 필터, 읽음 처리
3. **검토 이력 페이지** — 완료된 검토 목록 조회 (검토자용)
4. **감사 로그 페이지** — 시스템 활동 로그 조회 (관리자용)

### Priority 2 — 미구현 기능 (❌)
5. **내보내기 기능** — CSV/Excel/PDF 다운로드 (라이센스, 비용, 사용현황)
6. **신청서 취소** — 검토 시작 전 신청 취소 기능
7. **신청서 복사** — 기존 신청서를 복사하여 새 신청서 작성
8. **API Key 복사** — 클립보드 복사 버튼

### Priority 3 — 부분 구현 보완 (⚠️)
9. **피드백 상세 표시** — 편집 페이지에서 검토자 피드백 상세 내용 표시
10. **알림 시스템 연동** — 상태 변경 시 알림 자동 생성
