...
> 최종 업데이트: 2026-02-13 (과제원 일괄 신청 기능 추가)
...
| 과제원(팀원) 일괄 신청 | - | ✅ 완료 | Knox ID 검색/수동입력, 프로젝트별 과제원 관리, Step7 중복제거 요약 |
...
| 피드백 기반 수정/재제출 | FR-053 | ✅ 완료 | 피드백 상세 표시, 피드백 요청 단계로 복귀 (feedbackStage) |
...
| IT_ADMIN 환경 준비 전용 UI | - | ✅ 완료 | EnvPrepPanel, 환경별 체크리스트, 대상자별 환경 준비 체크, 전체 선택/해제 |
| LICENSE_MANAGER 라이센스 발급 전용 UI | - | ✅ 완료 | LicenseIssuancePanel, 대상자 목록 표시, 인원수 반영 비용 계산, 쿼터/유효기간 설정 |
| 검토 목록 대상 인원 표시 | - | ✅ 완료 | ReviewListItem.totalMemberCount, 검토 목록 테이블에 인원 수 표시 |
| 일괄 검토 처리 | FR-055 | ❌ 미구현 | |
