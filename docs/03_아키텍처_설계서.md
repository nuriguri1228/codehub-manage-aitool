...
│                                                                         │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐     │
│  │ Auth     │ │Application│ │ Review   │ │ API Key  │ │Monitoring│     │
│  │ Module   │ │ Module   │ │ Workflow │ │ Module   │ │ Module   │     │
│  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘     │
│       └─────────────┼───────────┼─────────────┼────────────┘           │
│                     │      Service Layer       │                        │
│                     └───────────┼──────────────┘                        │
...
│  │                     공통 인프라 계층                                │   │
│  │  ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐        │   │
│  │  │ Guards │ │Intercep│ │ Pipes  │ │ Logger │ │ Cache  │        │   │
│  │  └────────┘ └────────┘ └────────┘ └────────┘ └────────┘        │   │
│  └──────────────────────────────┼───────────────────────────────────┘   │
└─────────────────────────────────┼───────────────────────────────────────┘
                                  │
        ...
        │             │           │           │             │
┌───────┴───┐  ┌──────┴──┐  ┌────┴────┐ ┌────┴────┐ ┌─────┴─────┐
│PostgreSQL │  │  Redis   │  │  MinIO  │ │Elasti-  │ │  Message  │
│ (Primary  │  │ (Cache/  │  │ (File   │ │ search  │ │  Queue    │
│  DB)      │  │  Session)│  │ Storage)│ │ (Log)   │ │ (Bull MQ) │
└───────────┘  └─────────┘  └─────────┘ └─────────┘ └───────────┘

        ┌─────────────────────────────────────────────────────┐
        │                외부 시스템 연동                       │
        │  ┌──────┐ ┌──────────┐ ┌──────┐ ┌───────┐ ┌──────┐ │
        │  │ SSO  │ │ HR시스템  │ │Claude│ │Anti-  │ │메신저 │ │
        │  │(SAML)│ │ (REST)   │ │ API  │ │gravity│ │(알림) │ │
        │  └──────┘ └──────────┘ └──────┘ └───────┘ └──────┘ │
        └─────────────────────────────────────────────────────┘
```
...
```
┌──────────────┐       ┌──────────────────┐       ┌──────────────┐
│    users     │       │   applications   │       │   ai_tools   │
├──────────────┤       ├──────────────────┤       ├──────────────┤
│ PK id        │──┐    │ PK id            │    ┌──│ PK id        │
│ employee_id  │  │    │ FK user_id       │──┐ │  │ name         │
│ name         │  ├───>│ FK ai_tool_id    │──┼─┘  │ vendor       │
│ email        │  │    │ FK environment_id│──┼─┐  │ description  │
│ department   │  │    │ status           │  │ │  │ is_active    │
│ position     │  │    │ purpose          │  │ │  └──────────────┘
│ phone        │  │    │ created_at       │  │ │
│ role         │  │    │ updated_at       │  │ │  ┌──────────────┐
│ is_active    │  │    └────────┬─────────┘  │ │  │ environments │
└──────────────┘  │             │             │ │  ├──────────────┤
                  │             │             │ └─>│ PK id        │
                  │    ┌────────┼─────────┐   │    │ name         │
                  │    │        │         │   │    │ description  │
...
┌──────────────────┐     ┌──────────────────┐     ┌──────────────────┐
│ review_workflows │     │  review_stages   │     │review_feedbacks  │
├──────────────────┤     ├──────────────────┤     ├──────────────────┤
│ PK id            │──┐  │ PK id            │──┐  │ PK id            │
│ FK application_id│  └─>│ FK workflow_id   │  └─>│ FK stage_id      │
│ current_stage    │     │ stage_type       │     │ FK reviewer_id   │
│ status           │     │ FK reviewer_id   │     │ content          │
...
┌──────────────────┐     ┌──────────────────┐     ┌──────────────────┐
│    api_keys      │     │    licenses      │     │security_agreements│
├──────────────────┤     ├──────────────────┤     ├──────────────────┤
│ PK id            │     │ PK id            │     │ PK id            │
│ FK application_id│     │ FK application_id│     │ FK application_id│
│ FK user_id       │     │ FK user_id       │     │ FK user_id       │
│ key_hash         │     │ FK ai_tool_id    │     │ agreement_version│
│ key_prefix       │     │ status           │     │ agreed_at        │
│ status           │     │ issued_at        │     │ ip_address       │
│ issued_at        │     │ expires_at       │     │ signature_data   │
...
┌──────────────────┐     ┌──────────────────┐     ┌──────────────────┐
│   attachments    │     │   usage_logs     │     │  notifications   │
├──────────────────┤     ├──────────────────┤     ├──────────────────┤
│ PK id            │     │ PK id            │     │ PK id            │
│ FK application_id│     │ FK user_id       │     │ FK user_id       │
│ file_name        │     │ FK api_key_id    │     │ title            │
│ file_path        │     │ FK ai_tool_id    │     │ content          │
│ file_size        │     │ action           │     │ type             │
│ mime_type        │     │ tokens_used      │     │ is_read          │
│ uploaded_at      │     │ cost             │     │ link_url         │
...
| ai_tool_id | UUID | FK(ai_tools.id), NOT NULL | 신청 AI 도구 ID |
...
| status | ENUM('DRAFT','SUBMITTED','IN_REVIEW','REVISION_REQUESTED','APPROVED','REJECTED','CANCELLED') | NOT NULL, DEFAULT 'DRAFT' | 신청 상태 |
...
│         └── [수정 요청] ──> (반복, 최대 N회)                       │
│                                                                  │
│  ※ 수정 요청 최대 횟수: 단계당 3회 (설정 가능)                     │
│  ※ 초과 시 자동 에스컬레이션 (상위 관리자 검토)                    │
└──────────────────────────────────────────────────────────────────┘
```
...
│                    (15분 만료)       (7일 만료)               │
│                    (메모리 저장)     (HttpOnly Cookie)        │
│                                                              │
│  ※ Refresh Token Rotation: 사용 시마다 새 토큰 발급           │
│  ※ Token Blacklist: 로그아웃 시 Redis에 블랙리스트 등록       │
└──────────────────────────────────────────────────────────────┘
```
...
│  1. 키 생성                                                   │
│     ┌──────────────────────────────────────────┐             │
│     │ Raw Key: sk-codehub-xxxxxxxxxxxxxxxx     │             │
...
│  │ SHA-256 해시   │   │ AES-256-GCM 암호화  │               │
│  │ (검증용, DB)   │   │ (복호화용, DB)       │               │
│  └────────────────┘   └─────────────────────┘               │
│                                                              │
│  2. 키 표시                                                   │
│     - 최초 발급 시에만 전체 키 1회 표시                         │
│     - 이후에는 key_prefix(앞 8자) + 마스킹만 표시              │
│     - 예: sk-codeh****************************               │
│                                                              │
│  3. 키 검증 흐름                                               │
│     Request Header → Extract Key → SHA-256 Hash              │
│     → DB에서 key_hash 조회 → 상태 확인 (ACTIVE/만료)          │
│     → 사용 로그 기록 → last_used_at 갱신                      │
│                                                              │
│  4. 키 암호화 키 관리                                          │
│     - Master Key: 환경변수 또는 HashiCorp Vault               │
│     - Key Encryption Key(KEK): Master Key에서 파생            │
│     - Data Encryption Key(DEK): 개별 API Key 암호화           │
│     - Key Rotation: 분기별 KEK 갱신                           │
└──────────────────────────────────────────────────────────────┘
```
...
│  │                                                          │  │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐   │  │
│  │  │ SSO Adapter  │  │ HR Adapter   │  │  AI Tool     │   │  │
│  │  │ (SAML 2.0)   │  │ (REST API)   │  │  Adapter     │   │  │
│  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘   │  │
│  │         │                 │                  │           │  │
│  │  ┌──────┴───────┐  ┌─────┴────────┐  ┌─────┴────────┐  │  │
│  │  │ Messenger    │  │ Notification │  │ Monitoring   │  │  │
│  │  │ Adapter      │  │ Hub          │  │ Collector    │  │  │
│  │  └──────┬───────┘  └──────────────┘  └──────┬───────┘  │  │
│  └─────────┼─────────────────────────────────────┼──────────┘  │
│            │                                     │             │
└────────────┼─────────────────────────────────────┼─────────────┘
     ...
     │               │                    │                 │
┌────┴────┐   ┌──────┴──────┐      ┌─────┴─────┐   ┌──────┴──────┐
│ 사내    │   │  사내 SSO   │      │ Claude    │   │ Antigravity │
│ 메신저   │   │  (SAML IdP) │      │ Code API  │   │    API      │
└─────────┘   └─────────────┘      └───────────┘   └─────────────┘
                    │
              ┌─────┴─────┐
              │ HR 시스템   │
...
│  │  │  (NestJS)        │││  (NestJS)        │            │   │
│  │  │  Replicas: 2~6   │││  Replicas: 2~6   │            │   │
│  │  └────────┬─────────┘│└────────┬─────────┘            │   │
│  │           └───────────┼────────┘                      │   │
│  │                       │                               │   │
│  │  ┌──────────────────┐ │ ┌──────────────────┐          │   │
│  │  │  Worker Pod      │ │ │  Scheduler Pod   │          │   │
...
│  └───────────────────────┼───────────────────────────────┘   │
│                          │                                   │
│  ┌─── Namespace: codehub-infra ─────────────────────────┐    │
│  │                                                      │    │
│  │  ┌────────────┐ ┌────────┐ ┌────────┐ ┌──────────┐  │    │
│  │  │ PostgreSQL │ │ Redis  │ │ MinIO  │ │Elastic-  │  │    │
│  │  │ (StatefulS)│ │(Clust.)│ │(Stafl.)│ │ search   │  │    │
│  │  └────────────┘ └────────┘ └────────┘ └──────────┘  │    │
│  │                                                      │    │
│  └──────────────────────────────────────────────────────┘    │
│                                                             │
...
│  - E2E Test (Playwright) - develop/main 브랜치만             │
│  - 코드 커버리지 리포트 (최소 80%)                             │
│                                                             │
│  Stage 3: Build                                             │
...
│  - release/* → staging 환경 자동 배포                        │
│  - main → production 수동 승인 후 배포                        │
│  - Helm Chart 기반 K8s 배포                                  │
