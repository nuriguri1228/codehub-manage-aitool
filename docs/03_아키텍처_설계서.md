...
| 작성일 | 2026-02-11 |
| 작성자 | 시스템 아키텍처팀 |
| 문서 상태 | 수정 (Mermaid 다이어그램 변환, 프론트엔드 타입 반영) |
...
| role | ENUM('APPLICANT','TEAM_LEAD','SECURITY_REVIEWER','IT_ADMIN','SYSTEM_ADMIN') | NOT NULL, DEFAULT 'APPLICANT' | 시스템 역할 |
...
| stage_type | ENUM('TEAM_REVIEW','SECURITY_REVIEW','ENV_PREPARATION','FINAL_APPROVAL','KEY_ISSUED') | NOT NULL | 단계 유형 |
...
| IT 관리자 | IT_ADMIN | 사용자 관리, AI 도구 관리, 모니터링, 최종 승인, 환경 준비 |
| 시스템 관리자 | SYSTEM_ADMIN | 시스템 전체 설정, 역할 관리, 감사 로그 전체 조회 |

#### 6.2.2 권한 매트릭스

| 기능 | APPLICANT | TEAM_LEAD | SECURITY_REVIEWER | IT_ADMIN | SYSTEM_ADMIN |
|------|:---------:|:---------:|:-----------------:|:--------:|:------------:|
| 신청서 작성 | O | O | - | O | O |
| 본인 신청 조회 | O | O | O | O | O |
