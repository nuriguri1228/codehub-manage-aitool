...
| 작성일 | 2026-02-13 |
| 작성자 | 시스템 아키텍처팀 |
| 문서 상태 | 수정 (역할 기반 검토 프로세스 변경: FINAL_APPROVAL 제거, LICENSE_MANAGER 역할 추가) |
...
| `review.approved` | 라이센스 발급 완료 (모든 검토 단계 통과) | API Key 발급 프로세스 시작 |
...
sequenceDiagram
    ...
    Note over Review: 모든 검토 단계 APPROVED (LICENSE_ISSUANCE까지 완료)
    ...
    Event-->>Audit: 감사 로그 기록
    Event-->>App: 상태 변경 (LICENSE_ISSUANCE → KEY_ISSUED)
```
...
erDiagram
    users {
        ...
        VARCHAR position
        VARCHAR phone
        UserRole role "APPLICANT | TEAM_LEAD | SECURITY_REVIEWER | IT_ADMIN | LICENSE_MANAGER | SYSTEM_ADMIN"
        BOOLEAN is_active
    ...
    applications {
        ...
        ApplicationStatus status "DRAFT | SUBMITTED | TEAM_REVIEW | SECURITY_REVIEW | ENV_PREPARATION | LICENSE_ISSUANCE | KEY_ISSUED | REJECTED | FEEDBACK_REQUESTED"
    ...
    review_stages {
        UUID id PK
        UUID workflow_id FK
        VARCHAR stage_type "TEAM_REVIEW | SECURITY_REVIEW | ENV_PREPARATION | LICENSE_ISSUANCE"
        INTEGER stage_order
        UUID reviewer_id FK
...
| role | ENUM('APPLICANT','TEAM_LEAD','SECURITY_REVIEWER','IT_ADMIN','LICENSE_MANAGER','SYSTEM_ADMIN') | NOT NULL, DEFAULT 'APPLICANT' | 시스템 역할 |
...
| status | ENUM('DRAFT','SUBMITTED','TEAM_REVIEW','SECURITY_REVIEW','ENV_PREPARATION','LICENSE_ISSUANCE','KEY_ISSUED','REJECTED','FEEDBACK_REQUESTED') | NOT NULL, DEFAULT 'DRAFT' | 신청 상태 |
| submitted_at | TIMESTAMP | NULL | 제출 시각 |
| completed_at | TIMESTAMP | NULL | 완료 시각 |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | 생성 시각 |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | 수정 시각 |

> **변경 사항**: `ai_tool_id` FK가 제거되고, `application_ai_tools` 중간 테이블을 통해 복수 AI 도구를 지원한다. `status` ENUM에서 `FINAL_APPROVAL`과 `APPROVED`가 제거되고 `LICENSE_ISSUANCE`가 추가되었다. `LICENSE_ISSUANCE` 승인 시 바로 `KEY_ISSUED`로 전이된다.
...
| stage_type | ENUM('TEAM_REVIEW','SECURITY_REVIEW','ENV_PREPARATION','LICENSE_ISSUANCE') | NOT NULL | 단계 유형 |
| stage_order | INTEGER | NOT NULL | 단계 순번 |
| reviewer_id | UUID | FK(users.id), NULL | 검토자 ID |
| status | ENUM('PENDING','IN_PROGRESS','APPROVED','REJECTED','FEEDBACK_REQUESTED','SKIPPED') | NOT NULL, DEFAULT 'PENDING' | 단계 상태 |
| comment | TEXT | NULL | 검토 의견 |
| checklist | JSONB | NULL | 검토 체크리스트 (ReviewChecklistItem[]) |
| started_at | TIMESTAMP | NULL | 시작 시각 |
| completed_at | TIMESTAMP | NULL | 완료 시각 |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | 생성 시각 |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | 수정 시각 |
...
stateDiagram-v2
    ...
    LICENSE_ISSUANCE --> KEY_ISSUED : 라이센스 발급 승인
    LICENSE_ISSUANCE --> REJECTED : reject()
...
> **참고**: 현재 프론트엔드 코드의 `ApplicationStatus`는 `DRAFT | SUBMITTED | TEAM_REVIEW | SECURITY_REVIEW | ENV_PREPARATION | LICENSE_ISSUANCE | KEY_ISSUED | REJECTED | FEEDBACK_REQUESTED`로 정의되어 있다. `FINAL_APPROVAL`과 `APPROVED` 상태는 제거되었으며, `LICENSE_ISSUANCE` 승인 시 바로 `KEY_ISSUED`로 전이된다.

#### 5.1.2 검토 워크플로우 상태 머신

```mermaid
flowchart LR
    subgraph Workflow["전체 워크플로우 흐름 (4단계 검토)"]
        TR["TEAM_REVIEW<br/>(팀 리드 검토)"] --> SR["SECURITY_REVIEW<br/>(보안 검토)"]
        SR --> EP["ENV_PREPARATION<br/>(환경 준비)"]
        EP --> LI["LICENSE_ISSUANCE<br/>(라이센스 발급)"]
        LI --> KI["KEY_ISSUED<br/>(키 발급)"]
    end
```
...
| IN_REVIEW | approve | KEY_ISSUED | 모든 검토 단계 APPROVED (LICENSE_ISSUANCE 포함) | API Key 발급 프로세스 시작 |
...
| 역할 | 코드 | 설명 |
|------|------|------|
| 신청자 | APPLICANT | AI 도구 사용 신청, 본인 신청 조회/수정 |
| 팀 리드 | TEAM_LEAD | 신청서 1차 검토(팀 검토), 피드백 작성 |
| 보안 검토자 | SECURITY_REVIEWER | 보안 검토 수행, 보안 관련 피드백 |
| IT 관리자 | IT_ADMIN | 물리 개발환경(VDI/Notebook)만 관리, 환경 준비 검토 |
| 라이센스 관리자 | LICENSE_MANAGER | 라이센스 발급 전담 (LICENSE_ISSUANCE 단계 검토) |
| 시스템 관리자 | SYSTEM_ADMIN | 전체 신청 프로세스 현황 관리, 모니터링/비용/도구/사용자/시스템 설정 전체 관리 (검토 단계 없음) |

#### 6.2.2 권한 매트릭스

| 기능 | APPLICANT | TEAM_LEAD | SECURITY_REVIEWER | IT_ADMIN | LICENSE_MANAGER | SYSTEM_ADMIN |
|------|:---------:|:---------:|:-----------------:|:--------:|:--------------:|:------------:|
| 신청서 작성 | O | O | - | - | - | - |
| 본인 신청 조회 | O | O | O | O | O | O |
| 전체 신청 조회 | - | O (배정건) | O (배정건) | O (배정건) | O (배정건) | O |
| 팀 검토 수행 (TEAM_REVIEW) | - | O | - | - | - | - |
| 보안 검토 수행 (SECURITY_REVIEW) | - | - | O | - | - | - |
| 환경 준비 검토 (ENV_PREPARATION) | - | - | - | O | - | - |
| 라이센스 발급 검토 (LICENSE_ISSUANCE) | - | - | - | - | O | - |
| 라이센스 관리 | - | - | - | - | O | O |
| API Key 발급 | - | - | - | - | - | O |
| 사용자 관리 | - | - | - | - | - | O |
| AI 도구 관리 | - | - | - | - | - | O |
...
> | v1.1 | 2026-02-11 | 시스템 아키텍처팀 | Mermaid 다이어그램 변환, 프론트엔드 타입 반영 |
> | v1.2 | 2026-02-13 | 시스템 아키텍처팀 | 역할 기반 검토 프로세스 변경: FINAL_APPROVAL 제거, LICENSE_MANAGER 역할 추가, 4단계 검토 플로우로 변경 |
