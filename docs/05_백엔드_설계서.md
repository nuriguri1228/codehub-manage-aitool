...
주요 기능:
- 사용자 프로필 조회/수정
- RBAC 역할 관리 (APPLICANT, TEAM_LEAD, SECURITY_REVIEWER, IT_ADMIN, LICENSE_MANAGER, SYSTEM_ADMIN)
...
주요 기능:
- 검토 워크플로우 생성/관리 (TEAM_REVIEW → SECURITY_REVIEW → ENV_PREPARATION → LICENSE_ISSUANCE)
- 단계별 검토자 배정 및 검토 수행
- 피드백 작성 (승인/반려/수정 요청)
- 상태 머신 기반 워크플로우 엔진
...
erDiagram
    User {
        ...
        varchar position
        varchar phone
        enum role "APPLICANT | TEAM_LEAD | SECURITY_REVIEWER | IT_ADMIN | LICENSE_MANAGER | SYSTEM_ADMIN"
        boolean is_active
    ...
    Application {
        ...
        text purpose
        enum status "DRAFT | SUBMITTED | TEAM_REVIEW | SECURITY_REVIEW | ENV_PREPARATION | LICENSE_ISSUANCE | KEY_ISSUED | REJECTED | FEEDBACK_REQUESTED"
    ...
    ReviewStage {
        uuid id PK
        uuid application_id FK
        varchar stage_name "TEAM_REVIEW | SECURITY_REVIEW | ENV_PREPARATION | LICENSE_ISSUANCE"
        integer stage_order
        uuid reviewer_id FK
...
stateDiagram-v2
    ...
    LICENSE_ISSUANCE --> REJECTED : 라이센스 발급 반려
    LICENSE_ISSUANCE --> FEEDBACK_REQUESTED : 수정 요청
    ...
    FEEDBACK_REQUESTED --> LICENSE_ISSUANCE : 재제출 (해당 단계로 복귀)
...
| ENV_PREPARATION | approve | LICENSE_ISSUANCE | 환경 준비 완료 | 라이센스 관리자에게 알림 |
| LICENSE_ISSUANCE | approve | KEY_ISSUED | 라이센스 관리자가 승인 | API Key 발급 프로세스 시작, 신청자에게 발급 알림, 라이센스 활성화 |
...
| `review.approved` | 라이센스 발급 승인 | API Key 발급 프로세스 시작, 라이센스 활성화 |
...
| Method | Endpoint | 설명 | 권한 |
|--------|----------|------|------|
| GET | `/api/v1/reviews` | 배정된 검토 목록 | TEAM_LEAD, SECURITY_REVIEWER, IT_ADMIN, LICENSE_MANAGER |
| GET | `/api/v1/reviews/:applicationId` | 검토 상세 (전체 단계 + 피드백) | 검토자, IT_ADMIN, LICENSE_MANAGER |
| POST | `/api/v1/reviews/:stageId/submit` | 검토 결과 제출 (승인/반려/수정요청) | 배정된 검토자 |
| GET | `/api/v1/reviews/stats` | 검토 통계 | TEAM_LEAD, SECURITY_REVIEWER, IT_ADMIN, LICENSE_MANAGER |
...
| Method | Endpoint | 설명 | 권한 |
|--------|----------|------|------|
| GET | `/api/v1/licenses` | 라이센스 목록 (필터/검색/페이지네이션) | LICENSE_MANAGER, SYSTEM_ADMIN |
| GET | `/api/v1/licenses/:id` | 라이센스 상세 | 본인, LICENSE_MANAGER, SYSTEM_ADMIN |
| PUT | `/api/v1/licenses/:id/status` | 라이센스 상태 변경 | LICENSE_MANAGER, SYSTEM_ADMIN |
...
sequenceDiagram
    ...
    API->>WF: 상태 전이 처리
    WF->>DB: ENV_PREPARATION → LICENSE_ISSUANCE
    WF->>Queue: review.stage.completed 이벤트
    Queue->>Notif: 라이센스 관리자에게 알림
    API-->>ITAdmin: 200 OK

    %% 5. 라이센스 발급
    LicMgr->>API: POST /reviews/:stageId/submit (APPROVED)
    API->>WF: 라이센스 발급 승인 처리
    WF->>DB: LICENSE_ISSUANCE → KEY_ISSUED
...
graph TD
    subgraph Roles["역할 (Roles)"]
        ...
        LICENSE_MANAGER["LICENSE_MANAGER<br/>라이센스 관리자"]
    ...
    subgraph Permissions["권한 (Permissions)"]
        ...
        P_REVIEW_ENV["환경 준비 검토"]
        P_REVIEW_LICENSE["라이센스 발급 검토"]
        P_KEY_ISSUE["API Key 발급"]
        P_LICENSE_MANAGE["라이센스 관리"]
    ...
    IT_ADMIN --> P_REVIEW_ENV
    IT_ADMIN --> P_AUDIT_LIMITED

    LICENSE_MANAGER --> P_REVIEW_LICENSE
    LICENSE_MANAGER --> P_KEY_ISSUE
    LICENSE_MANAGER --> P_LICENSE_MANAGE

    SYSTEM_ADMIN --> P_APP_CREATE
    SYSTEM_ADMIN --> P_APP_READ_ALL
    SYSTEM_ADMIN --> P_KEY_ISSUE
    SYSTEM_ADMIN --> P_LICENSE_MANAGE
